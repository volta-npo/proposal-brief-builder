export function validateSaasDefinition(saas) {
  ...
  for (const key of ['personas', 'journeys', 'features', 'experiments', 'risks', 'automations', 'dashboards', 'templates']) {
    if (!Array.isArray(saas[key]) || saas[key].length < 3) throw new Error(`standalone SaaS needs deep ${key} definitions`);
  }
  ...
  if (saas.features.length < 6) throw new Error('standalone SaaS needs a robust feature catalog');
  if (saas.journeys.length < 5) throw new Error('standalone SaaS needs an end-to-end customer journey');
  return true;
}
...
export function saasWarnings(score, controlScore, workspace) {
  ...
  if (plannedFeatures.length) warnings.push(`${plannedFeatures.length} product feature(s) still need pilot or live status.`);
  const openRisks = (workspace.risks || []).filter((risk) => risk.status !== 'mitigated');
  if (openRisks.length) warnings.push(`${openRisks.length} launch risk(s) still need mitigation evidence.`);
  return warnings;
}
