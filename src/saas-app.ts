...
function loadSaas() { try { const raw = localStorage.getItem(key); if (raw) return mergeWorkspace(createSaasWorkspace(saas), JSON.parse(raw)); } catch {} return createSaasWorkspace(saas); }
