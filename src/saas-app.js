...
function loadSaas() { try {
    const raw = localStorage.getItem(key);
    if (raw)
        return mergeWorkspace(createSaasWorkspace(saas), JSON.parse(raw));
}
catch { } return createSaasWorkspace(saas); }
...
function installSaas() {
    ...
    $('#saas-sample').addEventListener('click', () => { if (!confirm('Load SaaS pilot sample data? This overwrites only the SaaS command center workspace.'))
        return; workspace = applySaasSample(saas); saveSaas(); renderSaas(); });
...
function bindSaas() {
    ...
    $$('[data-saas-metric]').forEach((input) => input.addEventListener('input', (event) => { const metric = workspace.metrics.find((item) => item.id === event.target.dataset.id); if (!metric)
        return; metric[event.target.dataset.saasMetric] = Number(event.target.value); saveSaas(); renderSaas(); }));
    $$('[data-saas-playbook]').forEach((input) => input.addEventListener('input', (event) => { const playbook = workspace.playbooks.find((item) => item.id === event.target.dataset.id); if (!playbook)
        return; playbook[event.target.dataset.saasPlaybook] = event.target.value; saveSaas(); renderSaas(); }));
    $$('[data-saas-control]').forEach((input) => input.addEventListener('input', (event) => { const control = workspace.controls.find((item) => item.id === event.target.dataset.id); if (!control)
        return; control[event.target.dataset.saasControl] = event.target.value; saveSaas(); renderSaas(); }));
    $$('[data-saas-roadmap]').forEach((input) => input.addEventListener('input', (event) => { const item = workspace.roadmap.find((entry) => entry.id === event.target.dataset.id); if (!item)
        return; item[event.target.dataset.saasRoadmap] = event.target.value; saveSaas(); renderSaas(); }));
    $$('[data-saas-list]').forEach((input) => input.addEventListener('input', (event) => { const items = workspace[event.target.dataset.saasList] || []; const item = items.find((entry) => entry.id === event.target.dataset.id); if (!item)
        return; item[event.target.dataset.field] = event.target.value; saveSaas(); renderSaas(); }));
}
installSaas();
